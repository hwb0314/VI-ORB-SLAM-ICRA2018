...
public:
     ...
     */
    virtual void transformForMultiThread(const std::vector<TDescriptor>& features,
      BowVector &v, FeatureVector &fv, int levelsup, unsigned int &i_feature);

    virtual bool transformForMultiThread_1(const std::vector<TDescriptor>& features,
      BowVector &v, FeatureVector &fv, int levelsup, unsigned int &i_feature) const;

    void Tmp(unsigned int &i, int num);

    std::mutex mMutexIncrease;
    std::mutex mMutexConst;
...
//    unsigned int i=0;
//    std::thread t1([&] { Tmp(std::ref(i), 1); } );
//    std::thread t2([&] { Tmp(std::ref(i), 2); } );
//    std::thread t3([&] { Tmp(std::ref(i), 3); } );
//    t1.join();
//    t2.join();
//    t3.join();
//    while(i<100) {}
//    std::cout << "out function, i = " << i << std::endl;

    /// Multi threads
    {
        unsigned int features_size = features.size();

        std::thread thread1([&] { transformForMultiThread(features, v, fv, levelsup, std::ref(i_feature)); } );
        std::thread thread2([&] { transformForMultiThread(features, v, fv, levelsup, std::ref(i_feature)); } );
        std::thread thread3([&] { transformForMultiThread(features, v, fv, levelsup, std::ref(i_feature)); } );

        auto lambda = [&] {transformForMultiThread(features, v, fv, levelsup, std::ref(i_feature));};
        lambda();
        ...
        while(i_feature<features_size) {}   // wait until threads finished
    }
...
//        v.clear();
//        fv.clear();
//        if( !transformForMultiThread_1(features, v, fv, levelsup, i_feature) )
...
template<class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor,F>::Tmp(unsigned int &i, int num)
{

    while(i<100)
    {
        std::cout << "Thread " << num << ": i = " << i << std::endl;

        unique_lock<mutex> lock(mMutexIncrease);
        i++;
    }
}
...
/// for Multi threads
template<class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor,F>::transformForMultiThread(
  const std::vector<TDescriptor>& features,
  BowVector &v, FeatureVector &fv, int levelsup, unsigned int &i_feature)
{
    unsigned int features_size = features.size();
    ...
    while(i_feature<features_size)
    {

        unsigned int i = 0;
        {
            unique_lock<mutex> lock(mMutexIncrease);
//            std::cout << "Thread " << thread_num << ": i_feature = " << i_feature << std::endl;
            i = i_feature;
            i_feature ++;
        }
...
template<class TDescriptor, class F>
bool TemplatedVocabulary<TDescriptor,F>::transformForMultiThread_1(
  const std::vector<TDescriptor>& features,
  BowVector &v, FeatureVector &fv, int levelsup, unsigned int &i_feature) const
{
    unsigned int features_size = features.size();
    ...
    while(i_feature<features_size)
    {
        ...
        if(i>=features_size) return true;
